...
describe('StakingLsdProxy', () => {
  ...
  it('Should fail when stake zero DFI', async () => {
    await expect(proxyStakingLsd.stake({ value: 0 })).to.be.revertedWithCustomError(proxyStakingLsd, 'AMOUNT_IS_ZERO');
  });

  it('Should fail when withdraw zero amount', async () => {
    await expect(proxyStakingLsd.withdraw(0)).to.be.revertedWithCustomError(proxyStakingLsd, 'AMOUNT_IS_ZERO');
  });

  it('Should fail when withdraw before staking', async () => {
    await expect(proxyStakingLsd.withdraw(100)).to.be.revertedWithCustomError(proxyStakingLsd, 'INSUFFICIENT_AMOUNT');
  });
  ...
  describe('Wallet address change tests', () => {
    ...
    it('Unable to change wallet address if not DEFAULT_ADMIN_ROLE', async () => {
      ...
      await expect(proxyStakingLsd.connect(newSigner).changeWalletAddress(newSigner.address)).to.be.revertedWith(
