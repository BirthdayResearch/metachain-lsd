...
describe('MarbleLsdRoles', () => {
  ...
  it('Should fail to grant DEFAULT_ADMIN_ROLE by non default admin address', async () => {
    ...
    const adminRoleHash = await proxyMarbleLsd.DEFAULT_ADMIN_ROLE();
    await expect(proxyMarbleLsd.connect(signer).grantRole(adminRoleHash, signer.address)).to.be.revertedWith(
  ...
  it('Should fail to grant ADMINISTRATOR_ROLE by non default admin address', async () => {
    ...
    await expect(proxyMarbleLsd.connect(signer).grantRole(administratorRoleHash, signer.address)).to.be.revertedWith(
      `AccessControl: account ${signer.address.toLowerCase()} is missing role 0x${'0'.repeat(64)}`,
    );
    await expect(proxyMarbleLsd.connect(administratorSigner).grantRole(administratorRoleHash, signer.address)).to.be.revertedWith(
  ...
  it('Should fail to grant FINALIZE_ROLE by non default admin address', async () => {
    ...
    await expect(proxyMarbleLsd.connect(signer).grantRole(finalizeRoleHash, signer.address)).to.be.revertedWith(
      `AccessControl: account ${signer.address.toLowerCase()} is missing role 0x${'0'.repeat(64)}`,
    );
    await expect(proxyMarbleLsd.connect(rewardDistributerAndFinalizeSigner).grantRole(finalizeRoleHash, signer.address)).to.be.revertedWith(
  ...
  it('Should fail to grant REWARDS_DISTRIBUTER_ROLE by non default admin address', async () => {
    ...
    await expect(proxyMarbleLsd.connect(signer).grantRole(rewardDistributerRoleHash, signer.address)).to.be.revertedWith(
      `AccessControl: account ${signer.address.toLowerCase()} is missing role 0x${'0'.repeat(64)}`,
    );
    await expect(proxyMarbleLsd.connect(rewardDistributerAndFinalizeSigner).grantRole(rewardDistributerRoleHash, signer.address)).to.be.revertedWith(
      `AccessControl: account ${rewardDistributerAndFinalizeSigner.address.toLowerCase()} is missing role 0x${'0'.repeat(64)}`,
    );
  });
  ...
  it('Should grant role by default admin address for DEFAULT_ADMIN_ROLE', async () => {
    const signer = accounts[5];
    const adminRoleHash = await proxyMarbleLsd.DEFAULT_ADMIN_ROLE();
    await proxyMarbleLsd.connect(defaultAdminSigner).grantRole(adminRoleHash, signer.address);
    expect(await proxyMarbleLsd.hasRole(adminRoleHash, signer.address)).to.equal(true);
  });

  it('Should grant role by default admin address for ADMINISTRATOR_ROLE', async () => {
    const signer = accounts[5];
    const administratorRoleHash = await proxyMarbleLsd.ADMINISTRATOR_ROLE();
    await proxyMarbleLsd.connect(defaultAdminSigner).grantRole(administratorRoleHash, signer.address);
    expect(await proxyMarbleLsd.hasRole(administratorRoleHash, signer.address)).to.equal(true);
  });

  it('Should grant role by default admin address for FINALIZE_ROLE', async () => {
    const signer = accounts[5];
    const finalizeRoleHash = await proxyMarbleLsd.FINALIZE_ROLE();
    await proxyMarbleLsd.connect(defaultAdminSigner).grantRole(finalizeRoleHash, signer.address);
    expect(await proxyMarbleLsd.hasRole(finalizeRoleHash, signer.address)).to.equal(true);
  });

  it('Should grant role by default admin address for REWARDS_DISTRIBUTER_ROLE', async () => {
    const signer = accounts[5];
    const rewardDistributerRoleHash = await proxyMarbleLsd.REWARDS_DISTRIBUTER_ROLE();
    await proxyMarbleLsd.connect(defaultAdminSigner).grantRole(rewardDistributerRoleHash, signer.address);
    expect(await proxyMarbleLsd.hasRole(rewardDistributerRoleHash, signer.address)).to.equal(true);
  });
});
