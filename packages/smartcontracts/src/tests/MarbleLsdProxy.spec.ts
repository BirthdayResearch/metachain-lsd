...
describe('MarbleLsdProxy', () => {
  ...
  it('Should fail when deposit with zero receiver address', async () => {
    const signer = accounts[5]
    await expect(proxyMarbleLsd.connect(signer).deposit(ethers.ZeroAddress, { value: toWei('10') }))
  ...
  it('Should fail when deposit with less than min deposit amount', async () => {
    const signer = accounts[5]
    await expect(proxyMarbleLsd.connect(signer).deposit(signer.address, { value: 10 }))
    .to.be.revertedWithCustomError(proxyMarbleLsd, "LESS_THAN_MIN_DEPOSIT");
  });
  ...
  it('Should fail when request withdrawal with zero receiver address', async () => {
    const signer = accounts[5]
    await expect(proxyMarbleLsd.connect(signer).requestWithdrawal(toWei('10'), ethers.ZeroAddress))
    .to.be.revertedWithCustomError(proxyMarbleLsd, "ZERO_ADDRESS");
  });
  ...
  it('Should have correct unfinalized assets count and request number', async () => {
    ...
    const preFinalize = await proxyMarbleLsd.prefinalize([lastRequestId]);
    expect(preFinalize.assetsToLock).to.equal(new BigNumber(unfinalizedAssets?.assets?.toString()).plus(unfinalizedAssets?.fees?.toString()))
  })
  ...
  describe('Roles', () => {
    it('Should have admin role to defaultAdminSigner address', async () => {
      ...
      expect(await proxyMarbleLsd.hasRole(adminRoleHash, defaultAdminSigner.address))
        .to.equal(true);
    });
  
    it('Should have administrator role to administratorSigner address', async () => {
      const administratorRoleHash = await proxyMarbleLsd.ADMINISTRATOR_ROLE();
      expect(await proxyMarbleLsd.hasRole(administratorRoleHash, administratorSigner.address))
        .to.equal(true);
    });
    ...
    it('Should have reward distributer role to rewardDistributerAndFinalizeSigner address', async () => {
      ...
      expect(await proxyMarbleLsd.hasRole(rewardDistributerRoleHash, rewardDistributerAndFinalizeSigner.address))
        .to.equal(true);
    });
    ...
    it('Should have finalizer role to rewardDistributerAndFinalizeSigner address', async () => {
      ...
      expect(await proxyMarbleLsd.hasRole(finalizerRoleHash, rewardDistributerAndFinalizeSigner.address))
        .to.equal(true);
    });
