...
describe('MarbleLsdProxy', () => {
  ...
  it('Should fail when withdraw zero amount', async () => {
    const signer = accounts[4];
    await expect(proxyMarbleLsd.withdraw(0, signer.address)).to.be.revertedWithCustomError(
      proxyMarbleLsd,
      'AMOUNT_IS_ZERO',
    );
  });
  ...
  it('Should fail when withdraw with zero receiver address', async () => {
    const signer = accounts[4];
    await expect(proxyMarbleLsd.connect(signer).withdraw(10, ethers.ZeroAddress)).to.be.revertedWithCustomError(
      proxyMarbleLsd,
      'ZERO_ADDRESS',
    );
  });
  ...
  describe('Minimum deposit update tests', () => {
    it('Unable to update minimum deposit if new amount is 0', async () => {
      ...
      await expect(proxyMarbleLsd.connect(defaultAdminSigner).updateMinDeposit(0)).to.be.revertedWithCustomError(
        proxyMarbleLsd,
        'AMOUNT_IS_ZERO',
      );
    });
  ...
  describe('Pause Unpause deposit and withdraw', () => {
    ...
    it('Should fail withdraw when withdraw is paused', async () => {
      const amount = toWei('10');
      const signer = accounts[4];
      await expect(proxyMarbleLsd.connect(signer).withdraw(amount, signer.address)).to.be.revertedWithCustomError(
        proxyMarbleLsd,
        'WITHDRAWAL_PAUSED',
      );
    });
