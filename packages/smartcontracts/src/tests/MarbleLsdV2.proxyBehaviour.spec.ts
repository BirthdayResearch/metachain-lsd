...
describe('Proxy behavior', () => {
  ...
  it('Should not upgrade contract from non admin address', async () => {
    ...
    await expect(
      proxyMarbleLsd.connect(accounts[6]).upgradeToAndCall(marbleLsdV2UpgradeableAddress, encodedData),
    ).to.be.revertedWith(`AccessControl: account ${signer.address.toLowerCase()} is missing role ${adminRoleHash}`);
  });
  ...
  it("Should upgrade and test contract's functionality and storage slots", async () => {
    ...
    expect(
      await marbleLsdV2.hasRole(REWARDS_DISTRIBUTER_ROLE, await rewardDistributerAndFinalizeSigner.getAddress()),
    ).to.equal(true);
    const FINALIZE_ROLE = await marbleLsdV2.FINALIZE_ROLE();
    expect(await marbleLsdV2.hasRole(FINALIZE_ROLE, await rewardDistributerAndFinalizeSigner.getAddress())).to.equal(
      true,
    );
